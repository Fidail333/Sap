...
export function CatalogCard({ item }: { item: CatalogProductItem }) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] transition hover:-translate-y-1 hover:border-cyan-300/50">
      ...
      <div className="flex h-full flex-1 flex-col gap-4 p-5">
        <div className="catalog-card-top space-y-4">
          <div className="catalog-card-badges flex flex-wrap gap-2">
          {item.catalog_type === 'modules' ? item.badges.map((badge) => <Badge key={badge}>{badge}</Badge>) : <Badge>Премиальная серия</Badge>}
          </div>
          <h3 className="catalog-card-title text-xl font-semibold text-white">{item.name}</h3>
          <p className="catalog-card-description text-sm text-slate-300">{item.catalog_type === 'modules' ? item.short_description : item.description}</p>
        </div>
